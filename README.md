...
Pretraining on the QM9 dataset.
Fine-tuning on 6 separate benchmark datasets as well as QM9 for property prediction (regression and classification).
...
Pretrained checkpoints and training logs (via Git LFS or external links) Due to platform upload limitations, the 1D SMILES encoder (SMILES-BERT) must be downloaded separately.
