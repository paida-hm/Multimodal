...
#         sim_matrix = torch.einsum('ik,jk->ij', x1, x2)  # shape: [B, B]
#
#         x1_norm = x1.norm(dim=1)
#         x2_norm = x2.norm(dim=1)
#         sim_matrix = sim_matrix / torch.einsum('i,j->ij', x1_norm, x2_norm)  # cosine sim
#
#         if pos_mask is None:
#             pos_mask = torch.eye(batch_size, device=x1.device)  # identity matrix as positive mask
#
#         pos_sim = (pos_mask * sim_matrix).sum(dim=1)
#         pos_sim = (pos_sim + 1) / 2  # rescale to [0, 1]
#         return pos_sim.mean()
class PositiveSimilarity(nn.Module):
    """
        https://en.wikipedia.org/wiki/Cosine_similarity
