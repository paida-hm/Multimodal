...
class QM9Dataset(Dataset):
    ...
    def __init__(self, return_types: list = None,
                 ...
                 normalize: bool = True, device='cuda:0', num_radial: int = 6, transform=None,smiles_tokenizer=None, max_smiles_length=120, **kwargs):
        """
        新增参数：
        - smiles_tokenizer: 用于SMILES序列的编码工具（例如字符级别、BPE、片段化模型等）。
        - max_smiles_length: 设置SMILES的最大长度，用于统一SMILES的表示。
        """
        super(QM9Dataset, self).__init__()
        self.smiles_tokenizer = smiles_tokenizer
        self.max_smiles_length = max_smiles_length
        # 读取SMILES信息
        if 'smiles_token' in return_types:
            if self.smiles_tokenizer is None:
                raise ValueError("需要提供smiles_tokenizer以支持SMILES编码。")
        ...
        if 'smiles_str' in self.return_types:
            self.smiles_list = pd.read_csv(os.path.join(self.qm9_directory, self.raw_qm9_file))['smiles'].tolist()

        # if 'smiles_token' in self.return_types:
        #     self.smiles_sequence = data_dict['smiles_sequence']
    ...
    #     """
    #     使用 SMILES-BERT 或字符级 Tokenizer 将 SMILES 转换为张量。
    #     """
    #     if isinstance(self.smiles_tokenizer, BertTokenizer):
    #         # 使用 SMILES-BERT tokenizer 进行编码
    #         inputs = self.smiles_tokenizer(smiles, return_tensors="pt", truncation=True, padding='max_length',
    #                                        max_length=self.max_smiles_length)
    #         return inputs['input_ids'].squeeze(0)  # 获取 input_ids，去掉 batch 维度
    #     else:
    #         # 采用字符级 Tokenization
    #         tokenized = list(smiles)  # 将 SMILES 转换为字符列表
    #         token_to_id = {ch: idx for idx, ch in enumerate(sorted(set(tokenized)))}  # 创建字符到索引映射
    #         tokenized_ids = [token_to_id[ch] for ch in tokenized]  # 转换为索引
    #
    #         # 填充或截断
    #         if len(tokenized_ids) > self.max_smiles_length:
    #             tokenized_ids = tokenized_ids[:self.max_smiles_length]
    #         else:
    #             tokenized_ids += [0] * (self.max_smiles_length - len(tokenized_ids))  # 用 0 进行填充
    #         print("Tokenized SMILES:", tokenized)
    #         print("Tokenized type:", type(tokenized))
    #         return torch.tensor(tokenized_ids, dtype=torch.long)  # 确保是数值张量
    ...
    def __getitem__(self, idx):
        ...
        # print(f"getitem({idx}) 返回: {data}")
        # 添加原始 SMILES 字符串输出（用于 1D 模态）
        # if 'smiles_str' in self.return_types:
        #     smiles_str = self.smiles_list[idx]
        #     data.append(smiles_str)
        return tuple(data)
    ...
    def data_by_type(self, idx, return_type, e_start, e_end, start, n_atoms):
        ...
        # elif return_type == 'smiles_token':
        #     return self.smiles_sequence[idx]
        elif return_type == 'smiles_str':
    ...
    def process(self):
        ...
        #     smiles_emb = self.smiles_to_sequence(smiles)
        #     smiles_embeddings.append(smiles_emb)
        ...
        data_dict = {'mol_id': data_qm9['id'],
                     ...
                     # 'smiles_sequence': torch.stack(smiles_embeddings),
                     'smiles_list': list(molecules_df['smiles'])  # 新增：保存原始 SMILES 字符串
                     }
