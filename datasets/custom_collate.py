...
def graph_collate(batch):
    if isinstance(batch[0], tuple) and len(batch[0]) == 2:
        # 微调阶段，仅包含 (graph_2d, target)
        graphs_2d, targets = map(list, zip(*batch))
        batched_graph_2d = dgl.batch(graphs_2d)
        targets = torch.stack(targets).float()
        if len(targets.shape) == 1:
            targets = targets.unsqueeze(-1)
        return [batched_graph_2d],targets
    elif isinstance(batch[0], tuple) and len(batch[0]) >= 3:
        # 解包每个元素：graph_2d, graph_3d, smiles_str
        graphs_2d, graphs_3d, smiles_strs, *targets = map(list, zip(*batch))
        batched_graph_2d = dgl.batch(graphs_2d)
        batched_graph_3d = dgl.batch(graphs_3d)
        smiles_strs = list(smiles_strs)
        if targets:
            return [batched_graph_2d], [batched_graph_3d], torch.stack(*targets).float()
        else:
            return [batched_graph_2d], [batched_graph_3d], smiles_strs
...
#     batched_graph_3d = dgl.batch(graphs_3d)
#
#     # 返回结构和原样一致，但 batched 了图
#     if targets:
#         # targets 是 [[target1, target2, ...]]，只有一项的 list，所以取 targets[0]
#         target_tensor = torch.stack(targets[0]).float()
#         return batched_graph_2d, batched_graph_3d, smiles_strs, target_tensor
#     else:
#         return batched_graph_2d, batched_graph_3d, smiles_strs
