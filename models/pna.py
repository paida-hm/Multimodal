...
class PNALayer(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, in_dim_edges: int, aggregators: List[str], scalers: List[str],
        ...
        # expected_in_dim = (2 * in_dim + in_dim_edges + 1) if self.pairwise_distances else (2 * in_dim + in_dim_edges)
        # print(f"Expected in_dim: {expected_in_dim}, MLP input: {self.pretrans.fully_connected[0].linear.in_features}")
