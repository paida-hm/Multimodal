...
def cov_loss(x):
    ...
    x = x - x.mean(dim=0)
    cov = (x.T @ x) / (n - 1)
    I = torch.eye(d).to(x.device)
    return ((cov - I) ** 2).sum() / d

def uniformity_loss(x, y):
    x = F.normalize(x, dim=-1)
    y = F.normalize(y, dim=-1)
    sim = torch.pairwise_distance(x, y, p=2)
    return torch.log(torch.exp(-2 * sim).mean() + 1e-8)
...
# class MultimodalNTXentLoss(torch.nn.Module):
#     def __init__(self, norm: bool = True, tau: float = 0.5,
#                  uniformity_reg=0, variance_reg=0, covariance_reg=0,
#                  weight_1d2d=1.0, weight_1d3d=1.0, weight_2d3d=1.0):
#         super(MultimodalNTXentLoss, self).__init__()
#         self.norm = norm
#         self.tau = tau
#         self.uniformity_reg = uniformity_reg
#         self.variance_reg = variance_reg
#         self.covariance_reg = covariance_reg
#
#         self.w12 = weight_1d2d
#         self.w13 = weight_1d3d
#         self.w23 = weight_2d3d
#
#     def forward(self, z_1d, z_2d, z_3d):
#         """计算 1D vs 2D vs 3D 的 NTXent Loss"""
#
#         loss = 0.0
#         loss += self.w12 * self.ntxent_pair(z_1d, z_2d)
#         loss += self.w13 * self.ntxent_pair(z_1d, z_3d)
#         loss += self.w23 * self.ntxent_pair(z_2d, z_3d)
#         # 正则项对每个模态都生效
#         for z in [z_1d, z_2d, z_3d]:
#             if self.variance_reg > 0:
#                 loss += self.variance_reg * std_loss(z)
#             if self.covariance_reg > 0:
#                 loss += self.covariance_reg * cov_loss(z)
#         if self.uniformity_reg > 0:
#             loss += self.uniformity_reg * (uniformity_loss(z_1d, z_2d) +
#                                            uniformity_loss(z_1d, z_3d) +
#                                            uniformity_loss(z_2d, z_3d))
#         return loss
#
#     def ntxent_pair(self, z_a, z_b):
#         batch_size, _ = z_a.size()
#         sim_matrix = torch.einsum('ik,jk->ij', z_a, z_b)
#
#         if self.norm:
#             a_abs = z_a.norm(dim=1)
#             b_abs = z_b.norm(dim=1)
#             sim_matrix = sim_matrix / (torch.einsum('i,j->ij', a_abs, b_abs) + 1e-8)
#
#         sim_matrix = torch.exp(sim_matrix / self.tau)
#         pos_sim = torch.diagonal(sim_matrix)
#         loss = pos_sim / (sim_matrix.sum(dim=1) - pos_sim)
...
#     def __init__(self, tau_1d=0.5, tau_2d=0.5, tau_3d=0.5,
#                  uniformity_reg=0.1, variance_reg=0.1, covariance_reg=0.1,
#                  alignment_reg=1.0,
#                  weight_1d2d=1.0, weight_1d3d=1.0, weight_2d3d=1.0,
#                  norm=True):
#         super().__init__()
...
#         self.norm = norm
#
#     def forward(self, z1d, z2d, z3d):
#         loss = 0.0
#
#         # 主要对比损失
#         loss += self.w12 * self.ntxent_pair(z1d, z2d, tau=self.tau_1d)
#         loss += self.w13 * self.ntxent_pair(z1d, z3d, tau=self.tau_1d)
#         loss += self.w23 * self.ntxent_pair(z2d, z3d, tau=self.tau_2d)
#
#         # 对齐损失
#         loss += self.alignment_reg * alignment_loss(z2d, z3d)
#         loss += self.alignment_reg * alignment_loss(z1d, z2d)
#         loss += self.alignment_reg * alignment_loss(z1d, z3d)
#
#         # 正则项
...
#
#         if self.uniformity_reg > 0:
#             loss += self.uniformity_reg * uniformity_loss(z1d, z2d)
#             loss += self.uniformity_reg * uniformity_loss(z1d, z3d)
#             loss += self.uniformity_reg * uniformity_loss(z2d, z3d)
#
#         return loss
#
#     def ntxent_pair(self, z_a, z_b, tau=0.5):
...
#             sim_matrix = torch.matmul(z_a, z_b.T)
#
#         sim_matrix = torch.exp(sim_matrix / tau)
#         pos_sim = torch.diagonal(sim_matrix)
#         loss = pos_sim / (sim_matrix.sum(dim=1) - pos_sim + 1e-8)
#         return -torch.log(loss + 1e-8).mean()
class MultimodalNTXentLoss(nn.Module):
    def __init__(self, tau_1d=0.5, tau_2d=0.5, tau_3d=0.5,
                 uniformity_reg=0.1, variance_reg=0.1, covariance_reg=0.1,
                 alignment_reg=1.0,
    ...
    def forward(self, z1d, z2d, z3d):
        ...
        def ntxent_direct(z_a, z_b, tau):
            ...
            sim_matrix = torch.exp(sim_matrix / tau)
            pos_sim = torch.diagonal(sim_matrix)
            denom = sim_matrix.sum(dim=1) - pos_sim + 1e-8
            return -torch.log(pos_sim / denom + 1e-8).mean()

        loss += self.w12 * ntxent_direct(z1d, z2d, self.tau_1d)
        loss += self.w13 * ntxent_direct(z1d, z3d, self.tau_1d)
        loss += self.w23 * ntxent_direct(z2d, z3d, self.tau_2d)
        ...
        # Regularizations
        for z in [z1d, z2d, z3d]:                #方差正则化: 确保每个模态的特征维度有足够方差，单模态层面防坍缩
            if self.variance_reg > 0:
                loss += self.variance_reg * std_loss(z)
            if self.covariance_reg > 0:
                loss += self.covariance_reg * cov_loss(z)    #协方差正则化确保每个模态内部的特征维度互相独立（防止冗余）

        if self.uniformity_reg > 0:                   #均匀性正则化： 确保不同模态在公共空间中保持均匀分布，促进跨模态对齐。
            loss += self.uniformity_reg * uniformity_loss(z1d, z2d)
            loss += self.uniformity_reg * uniformity_loss(z1d, z3d)
            loss += self.uniformity_reg * uniformity_loss(z2d, z3d)
